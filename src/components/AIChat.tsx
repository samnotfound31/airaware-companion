...
export default function AIChat({ isFloating = false }: AIChatProps) {
  ...
  if (isFloating && !isExpanded) {
    return (
      <button
        ...
        className="fixed bottom-24 right-6 z-50 glass-card rounded-full p-4 hover:scale-110 transition-transform"
  ...
  return (
    <div
      className={`${
        isFloating
          ? "fixed bottom-24 right-6 w-96 max-h-[600px] z-50 shadow-2xl"
          : "w-full h-full"
